...
from utils import check_social_media, scrape_github, scrape_kaskus, scrape_dicoding, scrape_zepeto, scrape_codewars, scrape_xmind
...
def check_account(site, target_account, pbar, found_counter):
    ...
    if result['status']:
        ...
        # If the site is Codewars, scrape additional information
        elif site_name == 'codewars':
            scraped_data = scrape_codewars(result['response'].text)
            for key, value in scraped_data.items():
                output += f"\n    {Style.BRIGHT} - [{Fore.LIGHTCYAN_EX}{key.capitalize()}{Style.RESET_ALL}{Style.BRIGHT}] {value}"

        # If the site is XMind, scrape additional information
        elif site_name == 'XMIND':
            scraped_data = scrape_xmind(result['response'].text)
            for key, value in scraped_data.items():
                output += f"\n    {Style.BRIGHT} - [{Fore.LIGHTCYAN_EX}{key.capitalize()}{Style.RESET_ALL}{Style.BRIGHT}] {value}"
